...
public class TabDictBrowser extends Activity {
	...
	private void fill(File[] files) {
		...
		items.add(getString(R.string.btn_back));
		for (File file : files) {
			if (file.isDirectory()) {
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> av, View v, int position, long arg3) {
				...
				// Log.v("path", path);
				// 如果不是返回键，则列出当前目录
				if (path.equals(getString(R.string.btn_back))) {
					// 当前如果不是根目录，则返回父目录列表
					if (!lastPath.equals("/")) {
			...
			public boolean onItemLongClick(AdapterView<?> av, View arg1, int position, long arg3) {
				...
				if (!path.equals(getString(R.string.btn_back))) {//返回按钮
					if (new File(path).listFiles() != null) {//有读写权限的目录
						...
						ad.setTitle(path);
						ad.setMessage(getString(R.string.dialog_msg_is_set_dict_dir));
						...
						ad.setPositiveButton(getString(R.string.btn_sure), new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {
								...
								Config.getConfig().setDictDir(TabDictBrowser.this, path);
								Log.i("path", path);
								finish();
							}
						});
						...
						// 取消按钮
						ad.setNegativeButton(getString(R.string.btn_cancel), new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog, int which) {
								dialog.cancel();
							}
