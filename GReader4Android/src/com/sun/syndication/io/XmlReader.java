...
public class XmlReader extends Reader {
	 ...
	 *             thrown if the charset encoding could not be determined
	 ...
	 * @throws IOException
	 *             thrown if there is a problem reading the stream of the
	 *             URLConnection.
	 * 
	 */
	 ...
	 *            content-type header to use for the resolution of the charset
	 *            encoding.
	 * @throws IOException
	 *             thrown if there is a problem reading the file.
	 * 
	 ...
	 * 
	 */
	public XmlReader(InputStream is, String httpContentType, boolean lenient, String defaultEncoding) throws IOException,
			XmlReaderException {
		_defaultEncoding = (defaultEncoding == null) ? _staticDefaultEncoding : defaultEncoding;
		try {
	...
	// InputStream is passed for XmlReaderException creation only
	private String calculateHttpEncoding(String cTMime, String cTEnc, String bomEnc, String xmlGuessEnc, String xmlEnc, InputStream is,
		...
		if (lenient & xmlEnc != null) {
			...
			if (appXml || textXml) {
				if (cTEnc == null) {
					...
					throw new XmlReaderException(HTTP_EX_1.format(new Object[] { cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc }), cTMime,
							cTEnc, bomEnc, xmlGuessEnc, xmlEnc, is);
				} else if (cTEnc.equals(UTF_16)) {
					if (bomEnc != null && bomEnc.startsWith(UTF_16)) {
						encoding = bomEnc;
					} else {
						throw new XmlReaderException(HTTP_EX_2.format(new Object[] { cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc }), cTMime,
								cTEnc, bomEnc, xmlGuessEnc, xmlEnc, is);
					}
				} else {
					encoding = cTEnc;
				...
				throw new XmlReaderException(HTTP_EX_3.format(new Object[] { cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc }), cTMime, cTEnc,
	...
	private static String getContentTypeEncoding(String httpContentType) {
		...
		if (httpContentType != null) {
			...
			if (encoding != null
					&& ((encoding.startsWith("\"") && encoding.endsWith("\"")) || (encoding.startsWith("'") && encoding.endsWith("'")))) {
	...
	private static boolean isAppXml(String mime) {
		return mime != null
				&& (mime.equals("application/xml") || mime.equals("application/xml-dtd")
						|| mime.equals("application/xml-external-parsed-entity") || (mime.startsWith("application/") && mime
						.endsWith("+xml")));
	}
	...
	private static boolean isTextXml(String mime) {
		return mime != null
				&& (mime.equals("text/xml") || mime.equals("text/xml-external-parsed-entity") || (mime.startsWith("text/") && mime
						.endsWith("+xml")));
	}
