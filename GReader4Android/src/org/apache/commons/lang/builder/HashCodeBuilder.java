...
public class HashCodeBuilder {
	...
	private static void reflectionAppend(Object object, Class clazz, HashCodeBuilder builder, boolean useTransients, String[] excludeFields) {
		...
		try {
			...
			for (int i = 0; i < fields.length; i++) {
				...
				if (!ArrayUtils.contains(excludeFields, field.getName()) && (field.getName().indexOf('$') == -1)
						&& (useTransients || !Modifier.isTransient(field.getModifiers())) && (!Modifier.isStatic(field.getModifiers()))) {
					try {
	 ...
	 * the likelyhood of bugs in mis-calling toHashCode() and the unlikelyness
	 * of it mattering what the hashCode for HashCodeBuilder itself is.
	 * 
	 * @return <code>hashCode</code> based on the fields appended
	 * @since 2.5
	 */
