...
public class CookieSpecBase implements CookieSpec {
	 ...
	 * 
	 * @param host
	 *            the host from which the <tt>Set-Cookie</tt> header was
	 *            received
	 * @param port
	 *            the port from which the <tt>Set-Cookie</tt> header was
	 *            received
	 * @param path
	 *            the path from which the <tt>Set-Cookie</tt> header was
	 *            received
	 * @param secure
	...
	public void validate(String host, int port, String path, boolean secure, final Cookie cookie) throws MalformedCookieException {
		...
		if (host.indexOf(".") >= 0) {
			...
			if (!host.endsWith(cookie.getDomain())) {
				...
				if (!host.equals(s)) {
					throw new MalformedCookieException("Illegal domain attribute \"" + cookie.getDomain() + "\". Domain of origin: \""
							+ host + "\"");
				}
			}
		} else {
