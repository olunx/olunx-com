 ...
 * processResponseHeaders(HttpState,HttpConnection)} to handle those headers</li>
 * </ul>
 * </p>
 * 
 * 
...
public abstract class HttpMethodBase implements HttpMethod {
	 ...
	 *            <tt>true</tt> if the method will automatically follow
	 ...
	 *            <tt>true</tt> to process authentication challenges
	...
	public byte[] getResponseBody() throws IOException {
		if (this.responseBody == null) {
			...
			if (instream != null) {
				...
				ByteArrayOutputStream outstream = new ByteArrayOutputStream(contentLength > 0 ? (int) contentLength
	...
	public byte[] getResponseBody(int maxlen) throws IOException {
		...
		if (this.responseBody == null) {
			...
			if (instream != null) {
				...
				LOG.debug("Buffering response body");
				ByteArrayOutputStream rawdata = new ByteArrayOutputStream(contentLength > 0 ? (int) contentLength
						: DEFAULT_INITIAL_BUFFER_SIZE);
	 ...
	 * 
	 * @param maxlen
	 *            the maximum content length to accept (number of bytes). Note
	 *            that, depending on the encoding, this is not equal to the
	 ...
	 * 
	 * @deprecated Use
	 *             {@link org.apache.commons.httpclient.params.HttpParams#setParameter(String, Object)}
	 ...
	 * @param conn
	 *            the {@link HttpConnection connection} to used to execute this
	 *            HTTP method. Must be non-null.
	 * 
	...
	/**
	 * Returns <tt>true</tt> if the HTTP method has been already
	 ...
	 * 
	 * @param state
	 *            HTTP state. TODO: to be removed in the future
	 * 
	 * @return cookie spec
	 */
	 ...
	 * <ol>
	 * <li>
	 * {@link #readStatusLine(HttpState,HttpConnection)} is invoked to read the
	 * request line.</li>
	 * <li>
	 * {@link #processStatusLine(HttpState,HttpConnection)} is invoked, allowing
	 * the method to process the status line if desired.</li>
	 * <li>
	 * {@link #readResponseHeaders(HttpState,HttpConnection)} is invoked to read
	 * the associated headers.</li>
	 * <li>
	 * {@link #processResponseHeaders(HttpState,HttpConnection)} is invoked,
	 * allowing the method to process the headers if desired.</li>
	 * <li>
	 * {@link #readResponseBody(HttpState,HttpConnection)} is invoked to read
	 * the associated body (if any).</li>
	 * <li>
	 * {@link #processResponseBody(HttpState,HttpConnection)} is invoked,
	 ...
	 * <ol>
	 * <li>
	 * {@link #writeRequestLine(HttpState, HttpConnection)} is invoked to write
	 * the request line.</li>
	 * <li>
	 * {@link #writeRequestHeaders(HttpState, HttpConnection)} is invoked to
	 * write the associated headers.</li>
	 * <li>
	 * <tt>\r\n</tt> is sent to close the head part of the request.</li>
	 * <li>
	 * {@link #writeRequestBody(HttpState, HttpConnection)} is invoked to write
	 * the body part of the request.</li>
	 * </ol>
	 * 
	 * <p>
	 * Subclasses may want to override one or more of the above methods to to
	...
	private String getRequestLine(HttpConnection conn) {
		return HttpMethodBase.generateRequestLine(conn, getName(), getPath(), getQueryString(), this.effectiveVersion.toString());
	}
	 ...
	 * authentication process. Otherwise returns <tt>null</tt>.
	 * 
	 ...
	 * process. Otherwise returns <tt>null</tt>.
	 * 
