...
public class ConnectMethod extends HttpMethodBase {
	 ...
	 * 
	 * @param method
	 *            the {@link HttpMethod method} to execute after connecting to
	 *            the server
	 */
	 ...
	 *            the client state
	 * @param conn
	 *            the {@link HttpConnection} the headers will eventually be
	 *            written to
	 * @throws IOException
			...
			if (connectionHeader != null) {
				if (connectionHeader.getValue().equalsIgnoreCase("close")) {
					if (LOG.isWarnEnabled()) {
						LOG.warn("Invalid header encountered '" + connectionHeader.toExternalForm() + "' in response "
