...
public class URI implements Cloneable, Comparable, Serializable {
	...
	/**
	 * Construct a URI as an escaped form of a character array. An URI can be
	 * placed within double-quotes or angle brackets like "http://test.com/" and
	 * &lt;http://test.com/&gt;
	 * 
	 * @param escaped
	 *            the URI character sequence
	...
	public URI(String scheme, String userinfo, String host, int port, String path, String query, String fragment) throws URIException {
		...
		this(scheme, (host == null) ? null : ((userinfo != null) ? userinfo + '@' : "") + host + ((port != -1) ? ":" + port : ""), path,
				query, fragment);
	}
	...
	protected void parseUriReference(String original, boolean escaped) throws URIException {
		...
		if (from < length) {
			...
			if (!_is_abs_path) {
				if (!escaped && prevalidate(tmp.substring(from, next), disallowed_rel_path) || escaped
						&& validate(tmp.substring(from, next).toCharArray(), rel_path)) {
					// Set flag
					_is_rel_path = true;
				} else if (!escaped && prevalidate(tmp.substring(from, next), disallowed_opaque_part) || escaped
	...
	protected void parseAuthority(String original, boolean escaped) throws URIException {
		...
		if (next != -1) { // neither -1 and 0
			...
			_userinfo = (escaped) ? original.substring(0, next).toCharArray() : encode(original.substring(0, next), allowed_userinfo,
					charset);
			from = next + 1;
		}
		...
		if (next >= from) {
			...
			// In IPv6reference, '[', ']' should be excluded
			_host = (escaped) ? original.substring(from, next).toCharArray() : encode(original.substring(from, next),
		...
		if (_is_reg_name) {
			...
			if (original.length() - 1 > next && hasPort && original.charAt(next) == ':') { // not
																							// empty
				from = next + 1;
				try {
	 ...
	 * 
	 * So API programmer must follow the following way: <code><pre>
	 ...
	 *  </pre></code>
	 * 
	 * The API programmer is responsible to set the correct charset. And each
	 * application should remember its own charset to support.
	 * 
	 * @param charset
	 ...
	 * <p>
	 * 
	 * @param escapedAuthority
	 *            the raw escaped authority
	 * @throws URIException
	 *             If {@link #parseAuthority(java.lang.String,boolean)} fails
	 * @throws NullPointerException
	 *             null authority
	 */
	...
	public void setRawPath(char[] escapedPath) throws URIException {
		...
		if (_is_net_path || _is_abs_path) {
			...
			if (at > 0 && !validate(escapedPath, 0, at - 1, rel_segment) && !validate(escapedPath, at, -1, abs_path) || at < 0
