...
public final class GoogleReaderDataProvider implements AuthenticationManager<GoogleCredentials> {
	private static final Log LOG = LogFactory.getLog(GoogleReaderDataProvider.class.getName());
	...
	public String getFeedItems(String feedUrl, Integer numberOfElements) throws GoogleReaderException {
		...
		if (numberOfElements != null) {
			parameters.add(new Parameter(Constants.PARAMETER_NUMBER_OF_RESULTS, numberOfElements));
		}
		return httpManager.get(url, parameters, true);
	}

	/**
	 * Get items from any feed by date.
	 * 
	 * @param feedUrl
	 * @param date
	 * @return
	...
	public String getFeedItemsFromDate(String feedUrl, String dateStr) throws GoogleReaderException {
		...
		List<Parameter> parameters = new ArrayList<Parameter>();
		if (dateStr != null) {
			parameters.add(new Parameter(Constants.PATAMETER_ORDER, "o"));
			parameters.add(new Parameter(Constants.PARAMETER_START_TIME, dateStr));
		}
	...
	public String getFeedsWithUnreadItems(OutputFormat outputFormat) throws GoogleReaderException {
		...
		if (outputFormat != null) {
			parameters.add(new Parameter(Constants.PARAMETER_OUTPUT_FORMAT, outputFormat.getFormat()));
		}
	...
	public String getItem(String itemId) throws GoogleReaderException {
		...
		return httpManager.post(Constants.URL_SEARCH_CONTENTS, parameters, true);
	}
	...
	public String getReadingList(Integer numberOfElements) throws GoogleReaderException {
		...
		return httpManager.get(Constants.URL_ITEMS_READING_LIST, parameters, true);
	}
	...
	public String getReadItems(Integer numberOfElements) throws GoogleReaderException {
		...
		return httpManager.get(Constants.URL_ITEMS_READ, parameters, true);
	}
	...
	public String getSharedItems(Integer numberOfElements) throws GoogleReaderException {
		...
		return httpManager.get(Constants.URL_ITEMS_SHARED, parameters, true);
	}
	...
	public String getStarredItems(Integer numberOfElements) throws GoogleReaderException {
		...
		return httpManager.get(Constants.URL_ITEMS_STARRED, parameters, true);
	}
	...
	public String getSubscriptions(OutputFormat outputFormat) throws GoogleReaderException {
		...
		return httpManager.get(Constants.URL_SUBSCRIPTION_LIST, parameters, true);
	 ...
	 *             If the user is not authenticated
	 */
	public String getUnreadItems(FeedDescriptor feed) throws GoogleReaderException {
	 ...
	 *             If the user is not authenticated
	 */
	public String getUnreadItems(String feedId, Integer numberOfElements) throws GoogleReaderException {
		...
		// parameters.add(new
		// Parameter(Constants.PARAMETER_STATE_FILTER,Constants.FILTER_CURRENT_USER_READ));
	 ...
	 * @throws GoogleReaderException
	 *             If a problem occured while encoding the string
	 */
	...
	public boolean login() throws AuthenticationException {
		...
		parameters.add(new Parameter("accountType", "HOSTED_OR_GOOGLE"));// TODO
																			// move
																			// to
																			// Constants
		...
		try {
			result = httpManager.post(Constants.URL_LOGIN, parameters, true);
			// LOG.debug("Login result: "+result);
			// Example:
			// SID=BLABLABLABLABLAVERYLONGID
			// LSID=BLABLABLABLAVERYLONGABITDIFFERENTID
			// Parse the result
			String sid = result.substring(result.indexOf(Constants.SID_TAG) + Constants.SID_TAG.length(), result.indexOf('\n'));
			// LOG.debug("SID: "+sid);
			result = result.substring(result.indexOf('\n') + 1);
			String lSid = result.substring(result.indexOf(Constants.LSID_TAG) + Constants.LSID_TAG.length(), result.indexOf('\n'));
			result = result.substring(result.indexOf('\n') + 1);
			String auth = result.substring(result.indexOf(Constants.AUTH_TAG) + Constants.AUTH_TAG.length(), result.indexOf('\n'));
			// LOG.debug("LSID: "+lSid);
			httpManager.clearCookies(); // just to be sure...
			/*
			 * httpManager.addCookie(new Cookie(Constants.COOKIE_DOMAIN,
			 * Constants.SID, sid, Constants.COOKIE_PATH,
			 * Constants.COOKIE_MAX_AGE, Constants.COOKIE_SECURE));
			 */
	 ...
	 *             If the user is not authenticated
	 */
	public String search(String searchTerm, FeedDescriptor feed) throws GoogleReaderException {
	 ...
	 *             If the user is not authenticated
	 */
	public String search(String searchTerm, FeedDescriptor feed, Integer numberOfElements) throws GoogleReaderException {
	...
	public String search(String searchTerm, FeedDescriptor feed, Integer numberOfElements, OutputFormat outputFormat)
		...
		if (feed != null) {
			// The feed id needs to be encoded
			parameters.add(new Parameter(Constants.PARAMETER_SEARCH_SOURCE_FEED, urlEncode(feed.getId())));
		}
		if (numberOfElements != null) {
			parameters.add(new Parameter(Constants.PARAMETER_SEARCH_NUMBER_OF_ELEMENTS, numberOfElements));
		}
		if (outputFormat != null) {
			parameters.add(new Parameter(Constants.PARAMETER_OUTPUT_FORMAT, outputFormat.getFormat()));
		}
		return httpManager.get(Constants.URL_SEARCH_IDS, parameters, true);
	}
	 ...
	 *             If the user is not authenticated
	 */
	public String search(String searchTerm, FeedDescriptor feed, OutputFormat outputFormat) throws GoogleReaderException {
	 ...
	 *             If the user is not authenticated
	 */
	public String search(String searchTerm, Integer numberOfElements) throws GoogleReaderException {
	 ...
	 *             If the user is not authenticated
	 */
	public String search(String searchTerm, Integer numberOfElements, OutputFormat outputFormat) throws GoogleReaderException {
	 ...
	 *             If the user is not authenticated
	 */
	public String search(String searchTerm, OutputFormat outputFormat) throws GoogleReaderException {
	 ...
	 * Get the reading list for an authenticated user and a specific label
	 * 
	 * @parem label The label to get the reading list for (not the id but the
	 *        NAME)
	 * @param numberOfElements
	 *            How many items should be retrieved (default if null: 20)
	 * @return The reading list (Atom as an XML String)
	...
	public void markItemAsRead(String itemId, String feedId) throws GoogleReaderException {
		...
		// "user/05721618992712852750/state/com.google/read"));
	 ...
	 * 
	 * @param label
	 *            The Label String to mark as read
	 * @throws GoogleReaderException
	 *             If the user is not authenticated
	 */
	...
	/**
	 * The feed you want to subscribe to (no error if you are already subscribed
	 * to the given feed)
	 * 
	 * @param feed
	 *            The feed to subscribe to
	 * @param title
	 *            The title for the new subscription
	 * @throws GoogleReaderException
	 *             If the user is not authenticated
	 * @throws IllegalArgumentException
	 *             If the feed is null or if the title is null or if the title
	 *             is empty
	 */
	...
	/**
	 * The feed you want to unsubscribe from (no error if you are not subscribed
	 * to the given feed)
	 * 
	 * @param feed
	...
	/**
	 * The url or feed id (@see FeedDescriptor class) of a feed you want to
	 * subscribe to (no error if you are already subscribed to the given feed)
	 * 
	 * @param feedUrl
	 *            The url or feed id (@see FeedDescriptor class) to subscribe to
	 * @param title
	 *            The title for the new subscription
	 * @throws GoogleReaderException
	 *             If the user is not authenticated
	...
	/**
	 * The url or feed id (@see FeedDescriptor class) of a feed you want to
	 * unsubscribe from (no error if you are not already subscribed to the given
	 * feed)
	 * 
	 * @param feedUrl
	 *            The url or feed id (@see FeedDescriptor class) to unsubscribe
	 *            from
	 * @throws GoogleReaderException
	 *             If the user is not authenticated
	 */
	 ...
	 * 
	 * @param feedUrl
	 *            The url of the feed to subscribe to or to unsubscribe from
	 * @param title
	 *            The title for the subscription to add (can be null if you
	 *            remove a subscription)
	 * @param add
	 *            If true, the subscription is to be added; if false, it will be
	 *            removed
	 * @throws GoogleReaderException
	 *             If the user is not authenticated
	 * @throws IllegalArgumentException
	 *             If the feed is null or if you try to add a subscription but
	...
	private void editSubscription(String feedUrl, String title, boolean add) throws GoogleReaderException {
		...
		// If we received a normal url and not the feedId from a FeedDescriptor,
		// no problem
	 ...
	 *            remove a subscription)
	 * @param label
	 *            The label for the subscription to be added to
	 * @param add
	 *            If true, the subscription is to be added to label; if false,
	 *            it will be removed
	 * @throws GoogleReaderException
	 *             If the user is not authenticated
	 * @throws IllegalArgumentException
