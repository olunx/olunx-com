...
public final class GoogleReaderUtil {
	...
	private static FeedDescriptor getFeedDescriptor(Element e) {
		...
		for (Element o : (List<Element>) e.getChildren("list")) {
			if (o.getAttributeValue("name").equals("categories")) {
				for (Element category : (List<Element>) o.getChildren()) {
					label = new Label();
					for (Element it : (List<Element>) category.getChildren("string")) {
		...
		for (Element o : (List<Element>) e.getChildren("number")) {
			...
			} else if (o.getAttributeValue("name").equals("newestItemTimestampUsec")) {
	...
	public static Item getItemFromJson(String content) throws GoogleReaderException {
		...
		try {
			...
			if (!json.isNull("alternate")) {
				JSONObject properties = json.getJSONArray("alternate").getJSONObject(0);
			...
			if (!items.isNull("summary")) {
				...
				returnValue.setContentTextDirection(itemsSummary.getString("direction"));
			}
			JSONObject itemsAlternate = items.getJSONArray("alternate").getJSONObject(0);
			...
			returnValue.setPublishedOn(new Date(items.getLong("published") * 1000));
			...
			if (!items.isNull("mediaGroup")) {
				...
				// mediaGroup.getJSONArray("content").getJSONObject(0).getString("url"));
			...
			if (!json.isNull("self")) {
				...
				String numericId = requestString.substring(requestString.indexOf("?i=") + "?i=".length(), requestString.indexOf("&T="));
			...
			if (!json.isNull("tags")) {
				...
				for (int i = 0; i < labels.length(); i++) {
					...
					if (id.indexOf(Constants.ITEM_STATE) >= 0) {
						...
						// (state object?)
						tmp.setName(id.substring(id.indexOf(Constants.ITEM_STATE) + Constants.ITEM_STATE.length() + 1));
				...
				for (int i = 0; i < results.length(); i++) {
					if (!results.isNull(i)) {
						...
						if ("shuffle-token".equals(current.getString("id"))) {
							returnValue.setShuffleToken(current.getString("value"));
						} else if ("start-page".equals(current.getString("id"))) {
							returnValue.setStartPage(current.getString("value"));
						} else if ("confirm-mark-as-read".equals(current.getString("id"))) {
	...
	public static List<FeedDescriptor> getFeedDescriptorsFromFeedsWithUnreadItemsXml(String xmlContent) throws GoogleReaderException {
		...
		try {
			...
			for (Element item : (List<Element>) xmlDocument.getRootElement().getChild("list").getChildren()) {
				...
				for (Element child : (List<Element>) item.getChildren()) {
					if ("id".equals(child.getAttributeValue("name"))) {
						...
						descriptor.setUnreadItems(Integer.valueOf(child.getValue()));
					} else if ("newestItemTimestampUsec".equals(child.getAttributeValue("name"))) {
