...
public class SparseFactory implements IFileFactory {
    ...
    public FileAccessBase newFileAccess(String string) throws IOException {
        if (string.startsWith("sparse://"))
            return new SparseFileAccess(string);
        else {
            FileAccessBase file = openNoneSparseFile(string + ".mul");
            if (file!=null && file.exists() && file.isFile())
                return new SparseFileAccess(string);
        }
        return null;
    }
    protected FileAccessBase openNoneSparseFile(String fileName) {
        for (int i = 0; i < FileFactory.getFactories().size(); ++i) {
            
            IFileFactory factory = (IFileFactory) FileFactory.getFactories().elementAt(i);
            FileAccessBase file;
            try {
                if (!(factory instanceof SparseFactory)) {
